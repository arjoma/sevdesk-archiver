...
def _validate_sidecar_shape(meta: Dict[str, Any], sidecar_name: str) -> List[str]:
    """Return human-readable shape errors for a sidecar (empty list = valid).

    Checks required keys and that ``pdf_filename`` matches the sidecar's own
    filename stem — guards against manual rename-on-disk drift.
    """
    errors: List[str] = []
    for k in _SIDECAR_REQUIRED_KEYS:
        if k not in meta:
            errors.append(f"missing key: {k}")
    expected_pdf = meta.get("pdf_filename")
    if expected_pdf and sidecar_name.endswith(".json"):
        stem = sidecar_name[:-5]
        if not str(expected_pdf).endswith(".pdf") or str(expected_pdf)[:-4] != stem:
            errors.append(
                f"pdf_filename {expected_pdf!r} does not match sidecar stem {stem!r}"
            )
    return errors
...
def verify_archive(target_dir: str, check_hashes: bool = True) -> Dict[str, Any]:
    """Deep integrity check of the archive.

    Checks run:
      * manifest.json exists and parses
      * every manifest entry's PDF / JSON is present on disk
        (``no_pdf``-flagged entries are exempt from the PDF check)
      * every file in ``files/`` is referenced by the manifest
      * every ``.pdf`` has a sibling ``.json`` and vice versa
      ...
      * when ``check_hashes=True``, every sidecar with a recorded
        ``pdf_hash`` re-hashes to the same value

    Returns a dict. Counts and lists are populated per check; ``errors`` only
    holds top-level failures (missing / unreadable manifest).
    """
    out: Dict[str, Any] = {
        "manifest_path": os.path.join(target_dir, MANIFEST_FILENAME),
        "manifest_count": 0,
    ...
    for name in sorted(on_disk):
        ...
        try:
            ...
            out["sidecar_errors"].append(f"{name}: unreadable ({e})")
            continue
        for err in _validate_sidecar_shape(meta, name):
            out["sidecar_errors"].append(f"{name}: {err}")
        sidecars[name] = meta
    ...
    for name in sorted(on_disk):
        if name.endswith(".pdf"):
            if name[:-4] + ".json" not in on_disk:
                out["unpaired_pdf"].append(name)
        elif name.endswith(".json"):
            if name[:-5] + ".pdf" in on_disk:
                continue
            meta = sidecars.get(name, {})
            doc = meta.get("document") or {}
            if not doc.get("_no_pdf"):
                out["unpaired_json"].append(name)
    ...
    for entry in entries:
        ...
        is_no_pdf = bool(entry.get("no_pdf"))
        if is_no_pdf:
            out["no_pdf_count"] += 1
        ...
        meta = sidecars.get(json_base)
        if meta is None:
            continue

        for m_field, s_field in (("id", "sevdesk_id"), ("type", "type")):
            mv = str(entry.get(m_field) or "")
            sv = str(meta.get(s_field) or "")
            if mv != sv:
                out["manifest_sidecar_mismatches"].append(
                    {
                        "file": json_base,
                        "field": m_field,
                        "manifest": mv,
        ...
        side_pdf = str(meta.get("pdf_filename") or "")
        if side_pdf and pdf_base and side_pdf != pdf_base:
            out["manifest_sidecar_mismatches"].append(
                {
                    "file": json_base,
                    "field": "pdf_filename",
        ...
        if check_hashes and not is_no_pdf and pdf_base and pdf_base in on_disk:
            recorded = meta.get("pdf_hash")
            if recorded:
                pdf_full = os.path.join(files_dir, pdf_base)
                try:
                    actual = _hash_file(pdf_full)
                except OSError as e:
                    out["errors"].append(f"hash read failed for {pdf_base}: {e}")
                    continue
                if actual == recorded:
                    out["hash_verified"] += 1
                else:
                    out["hash_mismatches"].append(
                        {
                            "file": f"{FILES_SUBDIR}/{pdf_base}",
                            "recorded": recorded,
                            "actual": actual,
                        }
                    )
...
def backfill_sidecar_hashes(target_dir: str) -> Dict[str, int]:
    """Add ``pdf_hash`` to every sidecar missing it by hashing the PDF on disk.

    Preserves all other fields, including ``archived_at``. Skips sidecars that
    already have a hash or whose document is flagged ``_no_pdf``.

    Returns counts: ``updated``, ``skipped``, ``missing_pdf``, ``errors``.
    """
    result = {"updated": 0, "skipped": 0, "missing_pdf": 0, "errors": 0}
    files_dir = _files_dir(target_dir)
    if not os.path.isdir(files_dir):
        return result
    ...
    for month_start, month_end in _iter_month_ranges(effective_after, effective_end):
        ...
        for doc_type in doc_types:
            ...
            for doc in docs:
                ...
                if need_json:
                    try:
                        if pdf_hash is None and existing is not None:
                            prev = (existing.get("meta") or {}).get("pdf_hash")
                            if prev:
                                pdf_hash = prev
                        if pdf_hash is None and os.path.exists(pdf_path):
                            try:
                                pdf_hash = _hash_file(pdf_path)
                            except OSError:
                                pass
                        _write_sidecar(
                            json_path, doc, doc_type, pdf_filename, pdf_hash=pdf_hash
                        )
                        if not need_pdf:
                            totals["refreshed"] += 1
                            yield {
