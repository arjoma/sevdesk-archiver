...
def verify(target, output_format, delete_orphans, yes):
    """Check manifest.json <-> files/ consistency.

    Reports entries whose PDF or JSON is missing from disk, plus orphan files
    on disk that aren't referenced by the manifest. Exits non-zero when any
    issue is found. With --delete-orphans, orphan files can be removed after
    confirmation (use --yes to skip the prompt).
    """
    target_dir = _resolve_target(target)
    report = archive_mod.verify_archive(target_dir)
